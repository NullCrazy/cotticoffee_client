...
mixin AutoScrollControllerMixin on ScrollController implements AutoScrollController {
  ...
  Future scrollToIndex(
    ...
    return co(
        ...
        () => _scrollToIndex(index,
            duration: duration, preferPosition: preferPosition, localOffset: offset));
  }
  ...
  Future _scrollToIndex(
    ...
    if (isIndexStateInLayoutRange(index)) {
      ...
      await _bringIntoViewportIfNeed(index, preferPosition, localOffset, (double offset) async {
      ...
      if (contains && hasClients) {
        await _bringIntoViewportIfNeed(
            index, preferPosition ?? _alignmentToPosition(lastScrollDirection), localOffset,
            (finalOffset) async {
  ...
  Future _bringIntoViewportIfNeed(int index, AutoScrollPosition? preferPosition, Rect? localOffset,
      Future Function(double offset) move) async {
    if (preferPosition != null) {
      double targetOffset = _directionalOffsetToRevealInViewport(
          index, localOffset, _positionToAlignment(preferPosition));
      ...
      final begin = _directionalOffsetToRevealInViewport(index, localOffset, 0);
      final end = _directionalOffsetToRevealInViewport(index, localOffset, 1);
  ...
  double _directionalOffsetToRevealInViewport(int index, Rect? localOffset, double alignment) {
    ...
    Rect rest() => localOffset ?? Rect.zero;
    ...
    if (tagOffsetInViewport == null) {
      ...
      if (alignment == 0.5) {
        ...
        return absoluteOffsetToViewport - beginGetter(viewportBoundaryGetter()) - beginGetter(rest());
      } else {
        return absoluteOffsetToViewport + endGetter(viewportBoundaryGetter()) + - beginGetter(rest());
      }
    }
  }
